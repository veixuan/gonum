...
// indexed into p.nodes by from to the node indexed by to. The len(seen) must
// match the number of nodes held by the receiver. The path parameter is the
// current working path and the results are written into paths.
